...
class HybridEngine:    
    def extract_unified_text(self, page: fitz.Page, clip_rect: fitz.Rect, 
        ...
        if debug_path:
            try:
                debug_pix = page.get_pixmap(dpi=target_dpi, clip=clip_rect, alpha=False)
                debug_pix.save(debug_path)
            except Exception as e:
                logger.warning(f"Could not save debug crop: {e}")

        # 1. Native Vector Check
        ...
        if native_char_count > 3 and not is_raster_embedded:
            ...
            if image_processor and ocr_engine:
                clean_img = image_processor.standardize_for_ocr(high_res_pix.tobytes("png"))
